...
contract product_identification
{
    ...
    struct product {  //productstructure
      string brand;
      string product_Id;
    ...
    mapping(string  => string[])CustomerUid_SerialNo;//at testing assign something to it 
//reporting stolen,getting details of customer
    mapping(string => retailerstructure)RetailerId_RetailerStruct;
    mapping(string => string[]) productId_serialNo;//mapping of productid with serial number. initailized when a
                        //product is created
    mapping(string => product)serialNo_productstructure;//serial no to product structure
